...
exports.createKeyPair = async function createKeyPair(options = {}) {
  const opt = {
    type:'rsa',
    format: 'pem',
    length: 2048,
  ...
  if ((opt.publicKeyType === 'pkcs1' || opt.privateKeyType === 'pkcs1') && type !== 'rsa') {
    throw new Error('Key Type of "pkcs1" is only allowed with rsa keys');
  }
  if (opt.privateKeyType === 'sec1' && type !== 'ec') {
  ...
  const data = await promisify(crypto.generateKeyPair(opt.type, {
