...
exports.formatCookie = function(name, val, options = {}) {
  ...
  if (opt.maxAge) {
    opt.maxAge = Number(opt.maxAge);
    if (!(isNaN(opt.maxAge))) {
      name += '; Max-Age=' + Math.floor(opt.maxAge);
    } else {console.error(`MaxAge "${opt.maxAge}" is not a valid number`)}
  }
  ...
  if (opt.expires) {
    ...
    if (!(/invalid date/i.test(opt.expires.toString()))) {
      name += '; Expires=' + (opt.expires).toUTCString();
    } else { console.error('Date is invalid')}
  }
  if (opt.sameSite) {
