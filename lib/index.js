...
exports.AuthLambdaFunction = class AuthLambdaFunction {
    ...
    static init(event, callback, params) {
        const $this = new AuthLambdaFunction(event,callback,params);
        return $this.init();
    }
    ...
    get env() {
        ...
        if (!env) return null;
        for (const i in env) {
            if (env.hasOwnProperty(i) && env[i].startsWith('$')) {
                const v = env[i].substring(1);
                if (this.params[v]) {
                    env[i] = this.params[v];
    ...
    getCookies(token, creds, user, additionalCookies = []) {
        ...
        if (user) headers["set-cookie"].push({
            key: 'Set-Cookie',
            value: formatCookie('x-forwarded-user', user)
        });
        const env = this.env;
        if (env) headers["set-cookie"].push({
            key: 'Set-Cookie',
            value: formatCookie('WebIdentityParams', env)
        });
        return headers
    }
}